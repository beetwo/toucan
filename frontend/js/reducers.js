...
import index from "react-timeago";
...
function issues(state = [], action) {
    switch (action.type) {
        ...
        case RECEIVE_ISSUE:
            let fresh_issue = action.payload;
            const existing_issue_index = state.findIndex(
                issue => issue.id === fresh_issue.id
            );
            let old_issue = state[existing_issue_index];
            return [
                ...state.slice(0, existing_issue_index),
                {
                    ...old_issue,
                    ...fresh_issue
                },
                ...state.slice(existing_issue_index + 1)
            ];
        default:
            return state;
    }
...
function allUsers(state = [], action) {
    switch (action.type) {
        case RECEIVE_ISSUE:
            let users = new Set(
                action.payload.users.map(u => u.username) || []
            );
...
function allOrganisations(state = [], action) {
    switch (action.type) {
        case RECEIVE_ISSUE:
            ...
            return state;
        case RECEIVE_ISSUES:
            let issues_with_orgs = action.issues.filter(
                i => i.organisation !== null
            );
            let org_slugs = issues_with_orgs.map(
                i => i.organisation.short_name
            );
